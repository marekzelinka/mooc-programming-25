...
while running:
    ...
    if robot_moving_dir[0] and robot_pos.y > 0:
        robot_pos.y -= velocity * dt
    if robot_moving_dir[1] and robot_pos.x + robot.get_width() < screen_width:
        robot_pos.x += velocity * dt
    if robot_moving_dir[2] and robot_pos.y + robot.get_height() < screen_height:
        robot_pos.y += velocity * dt
    if robot_moving_dir[3] and robot_pos.x > 0:
        robot_pos.x -= velocity * dt
